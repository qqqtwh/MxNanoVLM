...
def parse_args():
    ...
    parser.add_argument("--checkpoints", type=str, default='checkpoints/demo',help="Path to a local checkpoint (directory or safetensors/pth). If omitted, we pull from HF.")
    parser.add_argument("--hf_model", type=str, default="lusxvr/nanoVLM-450M",help="HuggingFace repo ID to download from incase --checkpoint isnt set.")
...
def main():
    ...
    source = args.checkpoints if args.checkpoints else args.hf_model
    print(f"Loading weights from: {source}")
    model = VisionLanguageModel.from_pretrained(source).to(device)
    model.eval()

    # tokenizer = get_tokenizer(model.cfg.lm_tokenizer, model.cfg.vlm_extra_tokens)
    tokenizer = get_tokenizer(model.cfg.lm_tokenizer, model.cfg.vlm_extra_tokens, model.cfg.lm_chat_template,model.cfg.lm_cache_dir)
